...
<Note>
	**Required Parameters:** You must provide either (`startDate` AND `endDate`)
	OR (`sessionId` OR `sessionIds`) to filter the results.
	
	**Optional Filters:** `sessionIdsExclude` can be used with `startDate` and `endDate` to exclude specific sessions from the date range.
</Note>
...
<ParamField path="sessionIdsExclude" type="array">
	...
	startDate and endDate parameters to filter out specific sessions from a date range.
	Provide as comma-separated values (e.g., "sess1,sess2,sess3").
</ParamField>
